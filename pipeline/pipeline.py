...
class TextToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def __call__(
        ...
        init_latents = init_image.unsqueeze(0).unsqueeze(2).expand(-1, -1, num_frames, -1, -1)
        init_latents = self.encode(init_latents, batch_size)
        init_latents = init_latents.to(device)
        
        if init_latents.shape[0] != batch_size:
            init_latents = init_latents.repeat(batch_size, 1, 1, 1, 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                init_model_input = self.scheduler.scale_model_input(init_model_input, t)
