...
def custom_checkpoint(module, mode=None):
    ...
    if mode == 'temp_conv':
         def custom_forward(hidden_states, num_frames=None, init_image=None):
            inputs = use_temporal(module, num_frames, hidden_states)
            if inputs is None: inputs = module(
                hidden_states, 
                num_frames=num_frames,
...
def cross_attn_g_c(
    ...
    def ordered_g_c(idx):
        ...
        if idx == 3: return g_c(custom_checkpoint(temp_conv, mode='temp_conv'), 
            hidden_states, num_frames, init_image, use_reentrant=False
    )
...
def up_down_g_c(resnet, temp_conv, hidden_states, temb, num_frames, init_image):
    hidden_states = g_c(custom_checkpoint(resnet, mode='resnet'), hidden_states, temb, use_reentrant=False)
    hidden_states = g_c(custom_checkpoint(temp_conv, mode='temp_conv'), 
        hidden_states, num_frames, init_image, use_reentrant=False
    )
    return hidden_states
...
class UNetMidBlock3DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        if self.gradient_checkpointing:
            ...
            hidden_states = self.temp_convs[0](hidden_states, num_frames=num_frames, init_image=init_image)
        ...
        for attn, temp_attn, resnet, temp_conv in zip(
            ...
            if self.gradient_checkpointing:
                ...
                if num_frames > 1:
                    hidden_states = temp_conv(hidden_states, num_frames=num_frames, init_image=init_image)
...
class DownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None, num_frames=1, init_image=None):
        output_states = ()

        for resnet, temp_conv in zip(self.resnets, self.temp_convs):
            if self.gradient_checkpointing:
                hidden_states = up_down_g_c(resnet, temp_conv, hidden_states, temb, num_frames, init_image)
            else:
                hidden_states = resnet(hidden_states, temb)
...
class UpBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, res_hidden_states_tuple, temb=None, upsample_size=None, num_frames=1, init_image=None):
        for resnet, temp_conv in zip(self.resnets, self.temp_convs):
            # pop res hidden states
            res_hidden_states = res_hidden_states_tuple[-1]
            res_hidden_states_tuple = res_hidden_states_tuple[:-1]
            ...
            if self.gradient_checkpointing:
                hidden_states = up_down_g_c(resnet, temp_conv, hidden_states, temb, num_frames, init_image=init_image)
            else:
                hidden_states = resnet(hidden_states, temb)
